...
- Migrate to `asyncio`-based controller (Faucet, ONOS) for full OVS 3.x compatibility
- Install per-host forwarding rules post-discovery to reduce controller bottleneck
- Implement host **ageing** — remove stale entries after inactivity timeout
- Extend to **multi-switch topologies** with host mobility support
- Add a REST API to expose the host database externally
